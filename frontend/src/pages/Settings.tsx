...
import {
  ...
  MapPin, RefreshCw, ImageIcon, Wifi, Signal,
} from "lucide-react";
import { getAutoDownloadPrefs, setAutoDownloadPrefs, PREFS_EVENT, type AutoDownloadPrefs } from "@/lib/mediaPrefs";
...
export default function Settings() {
  ...
  // Auto-download prefs
  const [dlPrefs, setDlPrefsState] = useState<AutoDownloadPrefs>(getAutoDownloadPrefs);
  useEffect(() => {
    const handler = (e: Event) => setDlPrefsState((e as CustomEvent<AutoDownloadPrefs>).detail);
    window.addEventListener(PREFS_EVENT, handler);
    return () => window.removeEventListener(PREFS_EVENT, handler);
  }, []);
  const toggleDl = (key: keyof AutoDownloadPrefs) => {
    setAutoDownloadPrefs({ ...dlPrefs, [key]: !dlPrefs[key] });
  };
            ...
            <div className="flex items-center justify-between px-5 py-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">Load images & videos when scrolled into view</p>
                </div>
              </div>
              <button
                onClick={() => toggleDl("wifi")}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${dlPrefs.wifi ? "bg-primary" : "bg-muted"}`}
              >
                <span className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${dlPrefs.wifi ? "translate-x-6" : "translate-x-1"}`} />
              </button>
            </div>
            {/* Mobile data */}
            <div className="flex items-center justify-between px-5 py-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-foreground">Auto-download on Mobile Data</p>
                  <p className="text-xs text-muted-foreground mt-0.5">May use significant data</p>
                </div>
              </div>
              <button
                onClick={() => toggleDl("mobile")}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${dlPrefs.mobile ? "bg-primary" : "bg-muted"}`}
              >
                <span className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${dlPrefs.mobile ? "translate-x-6" : "translate-x-1"}`} />
              </button>
            </div>
          </div>
        </div>
